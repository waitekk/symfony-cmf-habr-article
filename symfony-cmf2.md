Продолжим разглядывать [Symfony CMF](http://cmf.sandbox.com), реализующей концепцию платформы для построения CMS из слабосвязанных компонентов. В [первой части][1] статьи мы подробно рассмотрели схему хранения и доступа к данным, во второй части нас ждет все остальное. 
...
Конфигурируется бандл [как обычно][13], но все параметры опциональны.
...
#### Блоки
Предусмотрен и бандл для работы с блоками. Блоки могут реализовывать какую-то логику или просто возвращать статичный контент, который можно вызвать в любом месте шаблона. BlockBundle основывается на [SonataBlockBundle][14] и где нужно, заменяет компоненты родительского бандла на свои, совместимые с PHPCR.
...
Можно создавать и [свои блоки][17].

[Механизм кэширования][18] вывода блоков работает поверх [SonataCacheBundle][19], правда, в BlockBundle отсутствуют адаптеры для MongoDB, Memcached и APC — придется довольствоваться Varnish или SSI.
...
Первый &mdash; [RDFa][20]-разметка. Это способ описать метаданные в HTML в стиле микроформатов, но с помощью атрибутов.
...
После этого код выше перестает быть «тупым» набором DOM-элементов, потому что информацию из атрибутов можно удобно извлечь в JS-код и связать ее с моделями и коллекциями Backbone.js при помощи [VIE.js][21] &mdash; это второй компонент.

Третьим в цепочке выступает [create.js][22], который избавляет нас от необходимости придумывать интерфейс редактирования.

![Схема работы create.js][23]
...
Ну и замыкает список [CreatePHP][25], библиотека, связывающая вызовы create.js и непосредственно бэкенд. Она отвечает за маппинг свойств модели на PHP к HTML-атрибутам и рендеринг сущности. Самые внимательные уже видели, что для CreatePHP существует Twig-расширение и его вызов красуется в первом же листинге этой статьи: передаем модель и указываем формат вывода. Красота.

Последние два компонента объединены для удобства в CreateBundle.

#### MediaBundle
Одной из бандлов самой минималистичной реализации является бандл для работы с медиа-объектами. Ими могут быть документы, двоичные файлы, MP3, видеоролики и еще чего душа пожелает. В текущей версии поддерживается загрузка картинок и скачивание файлов, все остальное писать руками. [SonataMediaBundle][26] может помочь, тем более что есть интеграция.
...
- `DirectoryInterface`: определеяется как директория;
...
Реализованы адаптеры для  медиа-браузера [elFinder][28], библиотеки [Gaufrette][29], дающей слой абстракции над файловой системой и [LiipImagine][30], которая упрощает манипуляции с картинками.
...
На этом все. Небольшой список полезных ссылок:
 ...
 - [Для желающих поучаствовать в разработке][40]
  ...
  [11]: http://habrastorage.org/files/ce0/7a4/cee/ce07a4cee26d4167822307cc35255a1d.png
  [12]: http://habrastorage.org/files/b3f/6e8/198/b3f6e8198f6445a88cab58fc05a111fa.png
  [13]: http://symfony.com/doc/current/cmf/reference/configuration/menu.html
  [14]: https://github.com/sonata-project/SonataBlockBundle
  [15]: http://habrastorage.org/files/7f3/bf4/f37/7f3bf4f37a9c4454a1141df11a716b60.png
  [16]: http://habrastorage.org/files/ac1/6d4/241/ac16d42416ca4152836e94b8ba03f809.png
  [17]: http://symfony.com/doc/current/cmf/bundles/block/create_your_own_blocks.html
  [18]: http://symfony.com/doc/current/cmf/bundles/block/cache.html
  [19]: https://github.com/vihuvac/SonataCacheBundle
  [20]: http://rdfa.info/
  [21]: https://github.com/bergie/VIE
  [22]: http://createjs.org/
  [23]: http://habrastorage.org/files/d95/21e/a9a/d9521ea9acd2475fa61c2bc8bdd8d324.png
  [24]: http://habrastorage.org/files/e1f/def/1d4/e1fdef1d46d34d2b866263d36b6460bd.png
  [25]: https://github.com/flack/createphp
  [26]: https://github.com/sonata-project/SonataMediaBundle
  [27]: http://habrastorage.org/files/b89/d57/dfc/b89d57dfc3fa411591ca614798077676.png
  [28]: http://elfinder.org/
  [29]: https://github.com/KnpLabs/Gaufrette
  [30]: https://github.com/liip/LiipImagineBundle
  [31]: http://habrastorage.org/files/e73/9a2/a91/e739a2a91d1d4941b63e76ba101986a8.png
  [32]: http://habrastorage.org/files/ef2/270/341/ef2270341650433e9c081db3d63159fb.png
  [33]: http://yadi.sk/d/_9tlJiqDH7SAL
  [34]: https://mega.co.nz/#!0FNBGIyT!R18SXj98cpZxgPAe3q94LFsw0H941Fkasrt29HOtFRg
