...
Сразу на ум приходит решение в лоб: создаем маршрут по умолчанию, (```/{url}``` с обязательным параметром ```url: .*```), один контроллер для всех запросов и в зависимости от содержимого перенаправляем запрос в другие контроллеры. При этом никто не отменяет конфликтов с другими роутами.
...
Решение получше предоставляет ```DoctrineRouter```. Он уже гораздо гибче, потому что ищет маршруты по URL в базе данных, при этом уже готова реализация для документов через PHPCR-ODM, а еще можно приделать любую свою. Маршрут может явно указывать контроллер, в противном случае используется ```ControllerResolver```, который попытается сам решить, какой контроллер будет обрабатывать запрос. Есть несколько встроенных распознавателей:
...
До кучи — поддерживается переадресация маршрутов (на другие роуты или абсолютные URL). ```DoctrineRouter``` хорошо справлялся со своей работой, пока не был помечен как устаревший со времен Symfony 2.1, но замену ему все равно нашли.

На данный момент для решения всех проблем с в Symfony CMF используются два компонента — ```ChainRouter``` и ```DynamicRouter```. Первый заменяет стандартный SF2-роутер и, несмотря на название, работу роутера (определение контроллера для обработки запроса) не выполняет. Вместо этого он дает возможность добавлять свои роутеры (сервисы роутеров ищутся по тегам) в список-цепочку, из которой запрос попытаются обработать все сконфигурированные роутеры, но — в порядке приоритета. 
...
Ну вот, у нас есть бесконечное количество доступных для использования роутеров. Какой брать для с динамических маршрутов? Тут в дело вступает ```DynamicRouter```. Его задачей является загрузка маршрутов из *провайдера*, провайдером может быть база данных. В стандартной поставке ```RoutingBundle``` есть реализации провайдеров для Doctrine PHPCR-ODM и Doctrine ORM.
