Продолжим разглядывать [Symfony CMF](http://cmf.sandbox.com). В [первой части][1] статьи мы подробно рассмотрели схему хранения и доступа к данным, во второй части нас ждет все остальное. 

Вторая часть статьи выходит со значительной задержкой из-за моей лени и проблем со здоровьем и интернетом. За эти пару месяцев система доросла до версии 1.0.0, и все последующие правки в master-ветке зачем-то ломают работу системы, не будучи документированными. На случай, если кто захочет ставить систему руками, помните &mdash; опирайтесь на стабильные версии, помеченные тегами.
...
![Sonata][4]
Знаменитый генератор админок [SonataAdminBundle][5] выполнят ровно ту же самую  функцию и в Symfony CMF, но только через специальную прослойку в виде [SonataDoctrinePhpcrAdminBundle][6]. Сделано это, чтобы оригинальный бандл мог абстрагироваться от хранилища данных.
...
В моей практике встречались случаи, когда среди статичного контента встречались разные формочки, которые гармоничней смотрелись в составе раздела сайта, нежели как отдельный компонент. Например, на одном сайте банка в URL текстового раздела ```/credits/cash``` добавлялся кредитный ```/calculator```, чтобы люди, прочитав необходимую информацию о кредитах, на месте могли посчитать себе нужные циферки.

Допустим, ```PageController``` обработает первую часть URL, что делать с калькулятором, который, очевидно, будет выступать отдельным контроллером? Дописать в конфиге ```pattern: /credits/cash/calculator``` и указать отдельный контроллер? Как-то некрасиво. Даже если расставить приоритеты между остальными маршрутами, совершенно очевидно, что гибкостью тут не пахнет — если изменится псевдоним в базе, руками придется править и конфиг.
...
Конфигурируется бандл [как обычно][11], но все параметры опциональны.
...
#### Блоки
Предусмотрен и бандл для работы с блоками. Блоки могут реализовывать какую-то логику или просто возвращать статичный контент, который можно вызвать в любом месте шаблона. BlockBundle основывается на [SonataBlockBundle][12] и где нужно, заменяет компоненты родительского бандла на свои, совместимые с PHPCR.
...
Можно создавать и [свои блоки][15].

[Механизм кэширования][16] вывода блоков работает поверх [SonataCacheBundle][17], правда, в BlockBundle отсутствуют адаптеры для MongoDB, Memcached и APC — придется довольствоваться Varnish или SSI.
...
Первый &mdash; [RDFa][18]-разметка. Это способ описать метаданные в HTML в стиле микроформатов, но с помощью атрибутов.
...
После этого код выше перестает быть «тупым» набором DOM-элементов, потому что информацию из атрибутов можно удобно извлечь в JS-код и связать ее с моделями и коллекциями Backbone.js при помощи [VIE.js][19] &mdash; это второй компонент.

Третьим в цепочке выступает [create.js][20], который избавляет нас от необходимости придумывать интерфейс редактирования.

![Схема работы create.js][21]
...
Ну и замыкает список [CreatePHP][23], библиотека, связывающая вызовы create.js и непосредственно бэкенд. Она отвечает за маппинг свойств модели на PHP к HTML-атрибутам и рендеринг сущности. Самые внимательные уже видели, что для CreatePHP существует Twig-расширение и его вызов красуется в первом же листинге этой статьи: передаем модель и указываем формат вывода. Красота.

Последние два компонента объединены для удобства в CreateBundle.

#### MediaBundle
Одной из бандлов самой минималистичной реализации является бандл для работы с медиа-объектами. Ими могут быть документы, двоичные файлы, MP3, видеоролики и еще чего душа пожелает. В текущей версии поддерживается загрузка картинок и скачивание файлов, все остальное писать руками. [SonataMediaBundle][24] может помочь, тем более что есть интеграция.

Бандл обеспечивает:

- базовые документы для простых моделей;
- базовые FormTypes для простых моделей;
...
Из-за обилия интерфейсов неважно, где в каком хранилище держать объекты (хотя в 1.0 работает только Doctrine PHPCR-ODM).
...
В документации часто встречаются обещания сделать X или Y *потом*. Если захочется кому-то порекламировать проект &mdash; убедить в целесообразности использования получится с трудом, я думаю. Не будет модных нынче eye-candy-простынок, обещающих, как легка и весела станет ваша жизнь после утановки Symfony CMF. В общем, «коробки», из которой можно достать привлекательную работающую систему, нет. ~~И наверно не будет~~
...
### Резюме
Итак, несмотря на многочисленные «но», проект выглядит многообещающим. На удивление удачно решены некоторые фундаментальные проблемы контент-менеджмента (например многоязычность и маршрутизация/меню). Разработка медленно ведется крайне ограниченным кругом людей, у которых и без того есть работа, поэтому сейчас лучшая помощь &mdash; форк на гитхабе и полезный пулл-реквест, будь то правка документации или исправление ошибок. Популяризация CMF только впереди (попробуйте поискать в сети материалы, их практически нет), вся надежда только на опенсорс и коммьюнити.

Использовать сейчас CMF в продакшене может быть рисково, но тем менее, стоит быть в курсе. Кто его знает,  может с [тех миллионов евро][28] что-нибудь сюда перепадет и через пару лет мы увидим замечательный продукт?

На этом все. Небольшой список полезных ссылок:

 - [Документация по CMF][29]
 - [Сайт проекта][30]
 - [Roadmap][31]
 - [Для желающих поучаствовать в разработке][32]
  ...
  [8]: http://symfony.com/doc/current/cmf/bundles/routing_auto/introduction.html
  [9]: http://habrastorage.org/files/ce0/7a4/cee/ce07a4cee26d4167822307cc35255a1d.png
  [10]: http://habrastorage.org/files/b3f/6e8/198/b3f6e8198f6445a88cab58fc05a111fa.png
  [11]: http://symfony.com/doc/current/cmf/reference/configuration/menu.html
  [12]: https://github.com/sonata-project/SonataBlockBundle
  [13]: http://habrastorage.org/files/7f3/bf4/f37/7f3bf4f37a9c4454a1141df11a716b60.png
  [14]: http://habrastorage.org/files/ac1/6d4/241/ac16d42416ca4152836e94b8ba03f809.png
  [15]: http://symfony.com/doc/current/cmf/bundles/block/create_your_own_blocks.html
  [16]: http://symfony.com/doc/current/cmf/bundles/block/cache.html
  [17]: https://github.com/vihuvac/SonataCacheBundle
  [18]: http://rdfa.info/
  [19]: https://github.com/bergie/VIE
  [20]: http://createjs.org/
  [21]: http://habrastorage.org/files/d95/21e/a9a/d9521ea9acd2475fa61c2bc8bdd8d324.png
  [22]: http://habrastorage.org/files/e1f/def/1d4/e1fdef1d46d34d2b866263d36b6460bd.png
  [23]: https://github.com/flack/createphp
  [24]: https://github.com/sonata-project/SonataMediaBundle
  [25]: http://yadi.sk/d/_9tlJiqDH7SAL
  [26]: https://mega.co.nz/#!0FNBGIyT!R18SXj98cpZxgPAe3q94LFsw0H941Fkasrt29HOtFRg
  [27]: http://nekaka.com/d/EpBFuiXio5
  [28]: http://fabien.potencier.org/article/71/sensiolabs-raises-5-million-euros-to-boost-the-symfony-ecosystem
  [29]: http://symfony.com/doc/current/cmf/index.html
  [30]: http://cmf.symfony.com/
  [31]: https://github.com/symfony-cmf/symfony-cmf/wiki/Roadmap
  [32]: http://symfony.com/doc/current/cmf/contributing/index.html